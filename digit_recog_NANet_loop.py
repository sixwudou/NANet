##########################################################
# DIGIT RECOGNITION - NANET LOOP
##########################################################
# Use NANet trained for max_steps to evaluate loss2
#using as input a CNN trained for CNNsteps
  ...
  if my_file.is_file():
        ...
        for step in range(CNNsteps):
          ...
          if step % 1000 == 0:
            ...
            loss_train = sess.run(loss2,feed_dict={x: batch[0], y_: batch[1], keep_prob: 1, RecF: 1.0})
            loss_test = sess.run(loss2,feed_dict={x: mnist.test.images, y_: mnist.test.labels, keep_prob: 1, RecF: 1.0})
            ...
            file = open("digit_recog_NANet_loop.txt","a")
            file.write("\n%f %f %f %f %f" %(step,loss_train,loss_test,train_accuracy,test_accuracy))
            print("Results successfully recorded!")
          train_step.run(feed_dict={x: batch[0], y_: batch[1], keep_prob: 0.5, RecF: 0})
